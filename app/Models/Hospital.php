...
class Hospital extends Model {
    ...
    private function getSupportedColumns() {
        ...
        $cols = [];
        $stmt = $this->db->query("SHOW COLUMNS FROM hospitals");
    ...
    public function add($data) {
        ...
        if (in_array('location_url', $cols, true)) {
            $insertCols[] = 'location_url';
            $params[] = $data['location_url'] ?? null;
        }
        ...
        $sql = "INSERT INTO hospitals (" . implode(',', $insertCols) . ") VALUES ($placeholders)";
    ...
    public function update($id, $data) {
        ...
        $params[] = $id;
        $sql = "UPDATE hospitals SET " . implode(', ', $set) . " WHERE id = ?";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute($params);
    }
